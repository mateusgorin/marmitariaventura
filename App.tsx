    ...
    <div className="relative min-h-screen flex flex-col items-center pt-8 pb-16 px-5 md:px-6">
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[300] bg-white animate-reveal flex flex-col">
          {/* Barra de Topo do Cardápio */}
          <div className="bg-[#ffe0c2] px-4 py-3 flex items-center justify-between border-b border-black/5">
            <div className="flex items-center gap-3">
              <img src={LOGO_URL} alt="Logo" className="w-8 h-8 rounded-full shadow-sm" />
              <span className="font-serif text-lg text-slate-900">Cardápio Ventura</span>
            </div>
            <button 
              onClick={() => setIsMenuOpen(false)}
              className="bg-slate-900 text-white px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider active:scale-90 transition-transform"
            >
              Voltar
            </button>
          </div>
          ...
          {/* Botão flutuante de fechar (Acessibilidade) */}
          <button 
            onClick={() => setIsMenuOpen(false)}
            className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-md text-white px-8 py-3 rounded-full shadow-2xl flex items-center gap-2 active:scale-95 transition-all border border-white/10"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
            <span className="text-sm font-black uppercase tracking-widest">Fechar</span>
          </button>
        </div>
      )}

      {/* HEADER: AURA & IDENTITY */}
      <header className={`w-full max-w-2xl flex flex-col items-center mb-8 md:mb-16 animate-reveal ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}>
        <div className="relative mb-10 md:mb-14 group">
          <div className="absolute inset-[-15%] bg-white rounded-full opacity-30 blur-[60px] md:blur-[80px]"></div>
          
          <div className="relative w-56 h-56 md:w-80 md:h-80 p-2.5 md:p-3 bg-white/10 backdrop-blur-2xl rounded-full shadow-2xl ring-1 ring-white/20 flex items-center justify-center overflow-hidden">
            <img 
              src={LOGO_URL} 
              alt="Logo Ventura" 
      ...
      {/* THE 4 MASTER BLOCKS */}
      <main className={`w-full max-w-4xl grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 justify-center transition-opacity ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}>
        
        {/* BLOCK 1: THE MENU (Agora abre o Iframe) */}
        <button 
        ...
        >
          <div className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center mb-2.5 md:mb-3 text-white transform group-hover:-translate-y-3 group-hover:scale-110 transition-all duration-500 relative z-10">
            <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
          </div>
        ...
        >
          <div className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center mb-2.5 md:mb-3 text-white transform group-hover:-translate-y-2 group-hover:scale-110 transition-all duration-500 relative z-10">
            <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
          </div>
          <div className="flex flex-col items-center space-y-0.5 md:space-y-1 relative z-10 px-1">
      ...
      {/* FOOTER */}
      <footer className={`mt-16 md:mt-20 flex flex-col items-center gap-8 animate-reveal delay-4 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}>
        <div className="flex flex-col items-center gap-4 text-center">
          <div className="w-10 h-[1px] bg-slate-900/10 rounded-full"></div>
          <a href="https://www.gorinsolucoes.com.br" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center group pb-4">
            <span className="text-[8px] font-bold uppercase tracking-[0.3em] text-slate-900/30">Site em desenvolvimento por</span>
            <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] text-slate-900/60">Gorin Soluções</span>
          </a>
        </div>
      </footer>
